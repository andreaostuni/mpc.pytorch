...
def LQRStep(
    ...
    # if u_lower is a float, expand it to a tensor [n_ctrl]
    if isinstance(u_lower, float):
        u_lower = torch.Tensor([u_lower]).expand(n_ctrl)
    # if u_upper is a float, expand it to a tensor [n_ctrl]
    if isinstance(u_upper, float):
        u_upper = torch.Tensor([u_upper]).expand(n_ctrl)
    ...
    def lqr_backward(ctx, C, c, F, f):
        ...
        for t in range(T - 1, -1, -1):
            ...
            if u_lower is None:
                ...
                lb = (
                    torch.tile(u_lower, (1, n_batch)).reshape(n_batch, -1).to(u.device)
                    if u_lower.ndimension() == 1
                    else u_lower[t] - u[t]
                )
                ub = (
                    torch.tile(u_upper, (1, n_batch)).reshape(n_batch, -1).to(u.device)
                    if u_upper.ndimension() == 1
                    else u_upper[t] - u[t]
                )
                ...
                kt, Qt_uu_free_LU, If, n_qp_iter = pnqp(
                    Qt_uu, qt_u, lb, ub, x_init=prev_kt, n_iter=20
                )
                if verbose > 1:
    ...
    def lqr_forward(ctx, x_init, C, c, F, f, Ks, ks):
        ...
        while (
            ...
            or (
                ...
                and torch.any((current_cost > old_cost)).cpu().item() == 1
            )
                ...
                if t < T - 1:
                    if isinstance(true_dynamics, mpc.LinDx):
                        ...
                        new_xtp1 = true_dynamics(
                            Variable(new_xt), Variable(new_ut)
                        ).data
        ...
        alphas[current_cost > old_cost] /= linesearch_decay
        alpha_du_norm = (
            (u - new_u).transpose(1, 2).contiguous().view(n_batch, -1).norm(2, 1)
        )
        ...
        return (
            ...
            new_u,
            LqrForOut(
                objs, full_du_norm, alpha_du_norm, torch.mean(alphas), current_cost
            ),
        )
    ...
    class LQRStepFn(Function):
        ...
        def forward(ctx, x_init, C, c, F, f=None):
            ...
            return (
                new_x,
                new_u,
                torch.Tensor([n_total_qp_iter]),
                for_out.costs,
                for_out.full_du_norm,
                for_out.mean_alphas,
            )
        ...
        def backward(ctx, dl_dx, dl_du, temp=None, temp2=None):
            ...
            if u_lower is None:
                ...
                I = (
                    torch.abs(
                        new_u
                        # - get_bound("lower", t, n_batch=n_batch, device=x_init.device)
                        - (
                            torch.tile(u_lower, (1, n_batch))
                            .reshape(n_batch, -1)
                            .to(x_init.device)
                    ...
                    torch.abs(
                        new_u
                        # - get_bound("upper", t, n_batch=n_batch, device=x_init.device)
                        - (
                            torch.tile(u_upper, (1, n_batch))
                            .reshape(n_batch, -1)
                            .to(x_init.device)
