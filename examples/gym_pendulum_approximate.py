...
# gym_log.min_level(gym_log.WARN)
...
if __name__ == "__main__":
    ...
    statev = torch.cat(
        (
            (torch.rand(Nv, 1, dtype=torch.double) - 0.5) * 2 * math.pi,
            (torch.rand(Nv, 1, dtype=torch.double) - 0.5) * 16,
        ),
        dim=1,
    )
    ...
    def train(new_data):
        ...
        Y = torch.cat(
            (dtheta.view(-1, 1), dtheta_dt.view(-1, 1)), dim=1
        )  # x' - x residual
        ...
        for epoch in range(TRAIN_EPOCH):
            ...
            optimizer.step()
            logger.debug(
                "ds %d epoch %d loss %f", dataset.shape[0], epoch, loss.mean().item()
            )
        ...
        logger.info(
            "Error with true dynamics theta %f theta_dt %f norm %f",
            dtheta.abs().mean(),
            dtheta_dt.abs().mean(),
            E.mean(),
        )
    ...
    env = wrappers.RecordVideo(
        env,
        video_folder="videos/pendulum_mpc_approx",
    ...
    if BOOT_STRAP_ITER:
        ...
        for i in range(BOOT_STRAP_ITER):
            pre_action_state = env.unwrapped.state
    ...
    try:
        for i in range(run_iter):
            ...
            np_action = action.detach().numpy().reshape(-1)
